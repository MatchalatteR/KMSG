...
class KMSG:
    def __init__(self, keg_list, asg_list, batch_size=8, max_seq_length=32, out_size=64, step=1,n_node=None, l2=None):
        ...
        self.keg_list = tf.constant(keg_list,dtype=tf.int32)
        self.asg_list = tf.constant(asg_list, dtype=tf.int32)
    ...
    def gat(self,adj_list, item, topK):

        neighbor = tf.gather_nd(adj_list, tf.reshape(item, [self.batch_size, -1, 1]))    # [batch_size,max_seq_length,topK]
    ...
    def kmsg(self, WSG_in, WSG_out, AIG_in, AIG_out, item, alias, mask, topK):
        item = tf.reshape(item, [self.batch_size, -1])
        alias = tf.reshape(alias, [self.batch_size, -1])
        mask = tf.reshape(mask, [self.batch_size, -1])

        fin_state = self.gat(self.keg_list, item, topK) + self.gat(self.asg_list, item, topK) + self.ggnn(WSG_in, WSG_out, item) + self.ggnn(AIG_in, AIG_out, item) # [16,7,64]
