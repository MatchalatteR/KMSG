...
from model.kmsg import *

class SharedTreePolicy:
    def __init__(self, keg_list, asg_list, layer=3, branch=32, hidden_size=64, learning_rate=1e-3,
                 seed=1,
        ...
        self.gnn = KMSG(keg_list=keg_list, asg_list=asg_list, n_node=self.branch ** self.layer,
    ...
    def mlp(self, id=None, softmax_activation=False):
        ...
        with tf.variable_scope('node_' + str(id), reuse=tf.AUTO_REUSE):
            state = self.gnn.kmsg(self.WSG_in, self.WSG_out, self.AIG_in, self.AIG_out, self.items, self.alias,
                                     self.mask, self.topK) # [batch_size, out_size] 
            ...
            if softmax_activation:
                ...
                outputs = l3 # [batch_size, branch]
        return outputs # [batch_size, branch]
    ...
    def forward_pass(self):
        ...
        return root_output #  [batch_size, branch**layer]
